...
func main() {
	...
	mux := mux.NewRouter()
	mux.HandleFunc("/WalletAccounts", allBooks(session)).Methods("GET")
	mux.HandleFunc("/CreateWalletAccount", addBook(session)).Methods("POST")
	//http.ListenAndServe("localhost:5000", mux)
	log.Fatal(http.ListenAndServe("localhost:2222", mux))
}
